...
func init() {
	...
	recursivelyIgnoredPkgs = append(recursivelyIgnoredPkgs, "go.innotegrity.dev/zerolog")
	args := flag.Args()
	if len(args) != 1 {
...
func main() {
	...
	// get the go.innotegrity.dev/zerolog.Event type
	event := getEvent(p)
	if event == nil {
		fmt.Fprintln(os.Stderr, "Error: go.innotegrity.dev/zerolog.Event declaration not found, maybe zerolog is not imported in the scanned package?")
		os.Exit(1)
	}

	// get all selections (function calls) with the go.innotegrity.dev/zerolog.Event (or pointer) receiver
...
func getEvent(p *loader.Program) types.Type {
	for _, pkg := range p.AllPackages {
		if strings.HasSuffix(pkg.Pkg.Path(), "go.innotegrity.dev/zerolog") {
